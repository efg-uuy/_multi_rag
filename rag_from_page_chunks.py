...
try:
    ...
    print("⚠️ transformers未安装，问题类型分类功能将禁用")

sys.path.append(os.path.dirname(__file__))
from get_text_embedding import get_text_embedding

# 确保依赖安装提示（新增transformers）
required_packages = {
    ...
    "transformers": "pip install transformers"  # 问题分类器依赖
}
...
def process_long_chunk(chunk: Dict[str, Any], max_tokens: int = 512, slide_step: int = 200) -> List[Dict[str, Any]]:
    ...
    for idx, sub_content in enumerate(sub_contents):
        ...
        # 新增：标记Chunk类型（文本/图表/表格）
        if "image" in sub_content.lower() or "图表" in sub_content:
            sub_metadata["chunk_type"] = "image"
        elif "table" in sub_content.lower() or "表格" in sub_content:
            sub_metadata["chunk_type"] = "table"
        else:
            sub_metadata["chunk_type"] = "text"
...
class QuestionTypeClassifier:
    def __init__(self):
        self.model_name = "uer/roberta-base-finetuned-chinanews-chinese"  # 轻量中文分类模型
        self.tokenizer = None
        self.model = None
        self.label_map = {0: "text", 1: "image", 2: "table"}  # 文本/图表/表格
        self._load_model()

    def _load_model(self):
        if not CLASSIFIER_AVAILABLE:
            print("⚠️ 无法加载问题分类器，将使用默认参数")
            return
        ...
        try:
            ...
            print(f"⚠️ 问题分类器加载失败：{str(e)}，将使用默认参数")
            self.tokenizer = None
            self.model = None

    def predict(self, question: str) -> str:
        if not CLASSIFIER_AVAILABLE or self.tokenizer is None or self.model is None:
            return "text"  # 默认为文本类型
        ...
        return self.label_map.get(predictions.item(), "text")


# ===================== 5. 混合检索与RRF融合 =====================
def rrf_fusion(vector_results, bm25_results, k=60):
    """独立的RRF融合函数，清晰分离融合逻辑"""
    vector_rank = {res["id"]: i + 1 for i, res in enumerate(vector_results)}
    bm25_rank = {res["id"]: i + 1 for i, res in enumerate(bm25_results)}
    all_ids = set(vector_rank.keys()).union(set(bm25_rank.keys()))

    rrf_scores = {}
    for doc_id in all_ids:
        v_rank = vector_rank.get(doc_id, float("inf"))
    ...
    id_to_res = {res["id"]: res for res in vector_results + bm25_results}
    return [id_to_res[doc_id] for doc_id in sorted_ids if doc_id in id_to_res]


def hybrid_retrieve(query, vector_retriever, bm25_retriever, vector_store, embedding_version,
                    question_type="text", top_k=5, candidate_top_k=10):
    """新增问题类型参数，用于动态调整检索策略"""
    # 向量检索结果
    ...
    # 基于问题类型的Chunk类型加权（新增核心逻辑）
    type_weights = {
        "text": {"text": 1.0, "image": 0.5, "table": 0.8},
        "image": {"text": 0.5, "image": 1.5, "table": 0.7},
        "table": {"text": 0.7, "image": 0.6, "table": 1.5}
    }
    weights = type_weights.get(question_type, type_weights["text"])

    # 应用类型权重重新排序
    weighted_results = []
    for idx, res in enumerate(fused_results):
        chunk_type = res["metadata"].get("chunk_type", "text")
        base_score = 1.0 / (idx + 1)  # 基础排名分
        weighted_score = base_score * weights[chunk_type]
        weighted_results.append((res, weighted_score))

    # 按加权分数排序并截断
    weighted_results.sort(key=lambda x: x[1], reverse=True)
    final_results = [res for res, _ in weighted_results[:top_k]]

    return final_results, version_matched


# ===================== 6. 向量库优化：MilvusVectorStore =====================
...
def build_prompt(query, context_chunks):
    context = "\n".join(
        [
            f"内容：{c['content']}\n来源：{c['metadata']['file_name']} P{c['metadata']['page']}（类型：{c['metadata']['chunk_type']}）"
...
class SimpleRAG:
    def __init__(self, chunk_json_path: str, batch_size: int = 32,
        ...
        self.question_classifier = QuestionTypeClassifier()  # 初始化问题分类器
    ...
    def setup(self):
        ...
        print(f"   - 问题分类：{'启用' if CLASSIFIER_AVAILABLE else '禁用'}")
    ...
    def retrieve_and_rerank(self, query: str, top_k: int = 3, candidate_top_k: int = 20) -> Tuple[
        ...
        # 新增：问题类型判断与参数调整
        question_type = self.question_classifier.predict(query)
        print(f"检测到问题类型：{question_type}，动态调整检索参数")

        # 图表类问题扩大候选集
        adjusted_candidate_k = 30 if question_type in ["image", "table"] else candidate_top_k
        print(f"候选集大小调整为：{adjusted_candidate_k}")
        ...
        hybrid_results, version_matched = hybrid_retrieve(
            ...
            top_k=adjusted_candidate_k,
            candidate_top_k=adjusted_candidate_k
        )

        # 检查是否需要兜底
        retrieval_type = "hybrid"
        final_candidates = hybrid_results
        if len(hybrid_results) < adjusted_candidate_k // 2:
            print(f"⚠️ 混合检索结果不足（{len(hybrid_results)}个），触发兜底检索")
            fallback_results = self.fallback_retriever.retrieve(query, top_k=adjusted_candidate_k)
            ...
            final_candidates = combined_results[:adjusted_candidate_k]
    ...
    def generate_answer(self, question: str, top_k: int = 3, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        log_metadata = {
            ...
            "question_type": self.question_classifier.predict(question)  # 记录问题类型
        }
        ...
        try:
            ...
            try:
                ...
                note = f"检索类型：{retrieval_type}，模型版本：{self.current_embedding_version}，问题类型：{log_metadata['question_type']}"
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("【测试2：图表类查询】")
    test_question2 = "图3.2展示的季度营收趋势如何？"
    ...
    test_question3 = "联邦制药2024年营业收入同比增长率为多少？"
